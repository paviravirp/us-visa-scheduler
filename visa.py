...
def reschedule(date):
    ...
    if time is None:
        title = "FAIL"
        msg = f"Time not found for date {date} to reschedule"
        return [title, msg]
    ...
    if(r.text.find('successfully scheduled') != -1):
        title = "SUCCESS"
        PRIOD_END = date
        print(f"The period end date is updated to {PRIOD_END}")
...
def get_time(date):
    ...
    if(len(data.get("available_times")) > 0):
...
def get_available_date(dates):
    ...
    seven_days_from_today_string = seven_days_from_today.strftime("%Y-%m-%d")
    PSD = datetime.strptime(seven_days_from_today_string, "%Y-%m-%d")
